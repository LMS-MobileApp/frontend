...
const Dashboard: React.FC = () => {
       ...
       if (token) {
         ...
         if (assignmentsResponse.data && Array.isArray(assignmentsResponse.data)) {
           ...
           if (assignmentsResponse.data.length > 0) {
             setAssignmentTitle(assignmentsResponse.data[0].title);
           }
         } else {
           setAssignments([]);
           setAssignmentTitle("");
           console.warn("No assignments data received or invalid format");
         }
 ...
 const handleCreateGroupChat = async () => {
   ...
   try {
     const token = await AsyncStorage.getItem("token");
     const payload = { name: newGroupName, assignmentTitle };
     const response = await axios.post(
